...
/* Modelo heredea toda las funcionalidaddes de observable*/
Model.prototype = new Observable()
/* Agrego más funcionalidades a mi prototipo
Haciendo que tenga una función llamada GOTO */
Model.prototype.goTo = function( index ){
	this.setActive( index )
}
/*Una función llamada SETSONGS que recibe un arreglo de canciones */
Model.prototype.setSongs = function( songs ){
	this.songs = songs
	/*dispara un evento que las canciones fueron cambiadas*/
	this.trigger('songs-changed', this.songs)
}
Model.prototype.getActual = function(){
	/*retorna la canción actual dentro del arreglo songs según el indice*/
	return this.songs[ this.actual ]
}
Model.prototype.getActualIndex = function(){
	/*retorna el indice de la canción actual*/
	return this.actual
}
...
/*para notificar si estamos reproduciendo o en pausa*/
Model.prototype.setPlaying = function( bool ){
	this._isPlaying = bool
	if( bool ) this.trigger('playing')
	else this.trigger('paused')
}
/* para saber si algo se está reproduciendo*/
Model.prototype.isPlaying = function(){
	return this._isPlaying
}
/*es una funcion que tiene un prototipo y tambien es observable*/
function View(){
	/* el self me permite llamar al view
	así las funciones no aputen al elemto window*/
	var self = this
	...
	/*cuando se ejecuta esto, lo hará en el entorno global, sobre window; y esto daría error. 
	así que la variable self, que llama a la función view evita esto.*/
